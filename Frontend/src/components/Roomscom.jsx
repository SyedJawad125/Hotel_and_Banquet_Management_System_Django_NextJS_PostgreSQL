...
const RoomsCom = () => {
  ...
  // room bookings (Room.is_occupied_today / get_upcoming_bookings), so
  // it's not something a user sets manually here.
  // code_name is now auto-generated by the backend on create (Room.save()),
  // mirroring booking_code on Booking/RoomBooking — it's read-only here.
  const emptyForm = {
    name_en: '', name_ar: '', capacity: '', capacity_count: '', badge: '', price_per_night: '',
  };
  const [form, setForm] = useState(emptyForm);
  const [imageFile, setImageFile] = useState(null);
  const [imagePreview, setImagePreview] = useState(null);
  ...
  const saveRoom = async (e) => {
    e.preventDefault();
    if (!form.name_en.trim() || !form.capacity.trim() || !form.price_per_night) {
      toast.error('Name (English), capacity, and price per night are required');
      return;
    }
  ...
  const exportCSV = () => {
    ...
    const headers = ['ID', 'Name (EN)', 'Name (AR)', 'Code Name', 'Capacity', 'Capacity Count', 'Price/Night', 'Badge', 'Status', 'Upcoming Dates', 'Booking Count'];
    const escape = (val) => {
      const s = val === null || val === undefined ? '' : String(val);
      return /[",\n]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s;
    };
    const formatUpcoming = (r) =>
      (r.upcoming_occupied_dates || [])
        .map((b) => `${b.check_in_date} to ${b.check_out_date}`)
        .join(' | ');
    const rows = filteredRecords.map((r) => [
      r.id, r.name_en, r.name_ar, r.code_name, r.capacity, r.capacity_count,
      r.price_per_night, r.badge, r.occupied ? 'Occupied' : 'Available', formatUpcoming(r), r.booking_count ?? 0,
    ]);
    const csv = [headers, ...rows].map((row) => row.map(escape).join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
            ...
            overflow: 'hidden', boxShadow: shadowCard,
              ...
              <div style={{ overflowX: 'auto' }}>
                <table className="rm-table" style={{ width: '100%', borderCollapse: 'collapse', minWidth: 940 }}>
                  <thead>
                    <tr style={{ background: ivory, borderBottom: `1px solid ${line}` }}>
                      {['ID', 'Room', 'Code', 'Capacity', 'Price/Night', 'Badge', 'Status', 'Bookings', ''].map((h, i) => (
                        <th key={i} style={{
                  ...
                  <tbody>
                    {filteredRecords.map((r, idx) => (
                      ...
                      <td data-label="ID" style={{ padding: '14px 18px', fontSize: 13, color: '#8A8270', fontWeight: 500 }}>
                        ...
                        <td data-label="Capacity" style={{ padding: '14px 18px', fontSize: 13.5, color: ink }}>{r.capacity}</td>
                        <td data-label="Price/Night" style={{ padding: '14px 18px', fontSize: 13.5, color: goldDeep, fontWeight: 600 }}>
                          SAR {Number(r.price_per_night || 0).toLocaleString()}
                        </td>
                        <td data-label="Badge" style={{ padding: '14px 18px' }}>
                          {r.badge ? (
                            <span style={{
            ...
            <div style={{
              ...
              {filteredRecords.map((r) => (
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12.5, color: '#A39C8A', marginBottom: 4 }}>
                    <MapPin size={13} color={gold} />
                    {r.capacity}
                  </div>

                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: '#A39C8A', marginBottom: 10 }}>
                    <span style={{
                      fontFamily: 'monospace', color: '#8A8270',
                      background: ivory, border: `1px solid ${lineSoft}`, borderRadius: 6, padding: '2px 7px',
                    }}>{r.code_name}</span>
                  </div>
      ...
      {modalOpen && (
        ...
        >
          <form onSubmit={saveRoom}>
            ...
            <FormGroup label="Price Per Night" required hint="Base price for one night (SAR)">
              <TextField
                type="number"
                min="0"
                step="0.01"
            ...
            {/* Code Name field removed — it's now auto-generated by the
                backend on create (Room.save()), mirroring booking_code on
                Booking/RoomBooking, and is read-only. Shown below for
                reference when editing an existing room. */}
            {editingRoom && (
              <FormGroup label="Code Name" hint="Auto-generated by the system — cannot be edited">
                <span style={{
                  display: 'inline-block', fontSize: 12.5, fontFamily: 'monospace', color: '#8A8270',
                  background: ivory, border: `1px solid ${line}`, borderRadius: 8, padding: '8px 12px',
                }}>
                  {editingRoom.code_name}
                </span>
              </FormGroup>
            )}
