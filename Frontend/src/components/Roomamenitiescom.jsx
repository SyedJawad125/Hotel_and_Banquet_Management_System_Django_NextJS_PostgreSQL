...
const responsiveStyles = `
  * { box-sizing: border-box; }
  @media (max-width: 768px) {
    .ra-container { padding: 65px 14px 40px !important; width: 100% !important; max-width: 100% !important; }
    .ra-header { flex-direction: column !important; align-items: flex-start !important; }
    .ra-title { font-size: 28px !important; }
    .ra-header-actions { width: 100% !important; }
    .ra-header-actions button { flex: 1 1 auto !important; justify-content: center !important; }

    .ra-stats-grid { grid-template-columns: repeat(2, minmax(0, 1fr)) !important; gap: 10px !important; }

    /* Outer table panel turns ivory so the white row-cards stand out against it */
    .ra-table-wrap { background: #F6F3EC !important; border: none !important; box-shadow: none !important; }

    /* Responsive table: hide header row, stack each cell as a labeled block */
    .ra-table thead { display: none !important; }
    .ra-table, .ra-table tbody, .ra-table tr, .ra-table td {
      display: block !important; width: 100% !important;
    }
    .ra-table { min-width: 0 !important; }
    .ra-table tr {
    ...
    .ra-table td.ra-empty-label { justify-content: flex-end !important; }
    .ra-table td.ra-empty-label::before { display: none !important; }

    /* Pagination bar inside the (now transparent) table wrap */
    .ra-pagination { background: #FFFFFF !important; border: 1px solid rgba(198,164,63,0.12) !important; border-radius: 14px !important; }
  }
`;
...
const RoomAmenitiesCom = () => {
  ...
  const emptyForm = {
    room: '', name_en: '', name_ar: '', description_en: '', description_ar: '',
  };
  ...
  const fetchAmenities = async () => {
    setLoading(true);
    try {
      const res = await AxiosInstance.get('/api/hotel/v1/room/amenity/', {
        params: {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((a) => {
      ...
      const roomIdMatch = (a.room?.id ?? a.room)?.toString() === q;
      const roomMatch = (a.room_name_en || a.room?.name_en)?.toLowerCase().includes(q);
      ...
      return idMatch || roomIdMatch || roomMatch || nameMatch || descMatch || createdByMatch;
    });
  }, [records, searchTerm]);

  const totalAmenities = count;
  const uniqueRooms = new Set(records.map((a) => a.room?.id ?? a.room)).size;
  ...
  const openEdit = (amenity) => {
    ...
    setForm({
      room: amenity.room?.id ?? amenity.room ?? '',
  ...
  const saveAmenity = async (e) => {
    e.preventDefault();
    if (!form.room || !form.name_en.trim()) {
      toast.error('Room and amenity name are required');
      return;
    }
    ...
    try {
      ...
      if (editingAmenity) {
        await AxiosInstance.patch(`/api/hotel/v1/room/amenity/?id=${editingAmenity.id}`, payload);
        toast.success('Amenity updated successfully');
      } else {
        await AxiosInstance.post('/api/hotel/v1/room/amenity/', payload);
  ...
  const deleteAmenity = async (amenity) => {
    ...
    try {
      const res = await AxiosInstance.delete(`/api/hotel/v1/room/amenity/?id=${amenity.id}`);
      if (res) {
  ...
  const exportCSV = () => {
    if (!filteredRecords.length) {
      toast.error('No amenities to export');
      return;
    }
    const headers = ['ID', 'Room ID', 'Room', 'Amenity Name (EN)', 'Amenity Name (AR)', 'Description (EN)', 'Description (AR)', 'Created By'];
    const escape = (val) => {
      const s = val === null || val === undefined ? '' : String(val);
      return /[",\n]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s;
    };
    const rows = filteredRecords.map((a) => [
      a.id, a.room?.id ?? a.room, a.room_name_en || a.room?.name_en, a.name_en, a.name_ar,
      a.description_en, a.description_ar, a.created_by_name,
    ]);
    ...
    link.href = url;
    link.download = `room-amenities-export-${stamp}.csv`;
  ...
  return (
    <div style={{ width: '100%', minHeight: '100%', background: '#F6F3EC', fontFamily: bodyFont }}>
      ...
      <div className="ra-container" style={{ maxWidth: 1280, margin: '0 auto', padding: '36px 28px 60px' }}>

        {/* Header */}
        <div className="ra-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 18, marginBottom: 30 }}>
          <div>
            <div style={{ fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase', color: gold, fontWeight: 700, marginBottom: 8 }}>
              Facility Management
            </div>
            <h1 className="ra-title" style={{ fontFamily: displayFont, fontSize: 38, color: ink, fontWeight: 600, lineHeight: 1.1 }}>
              Room Amenities
            </h1>
          </div>

          <div className="ra-header-actions" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button
              onClick={exportCSV}
              style={{
            ...
            {permissions.create_room_amenity && (
              <button
              onClick={openCreate}
              style={{
                display: 'flex', alignItems: 'center', gap: 8,
        ...
        {/* Stat strip */}
        <div className="ra-stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14, marginBottom: 26 }}>
          {[
            { label: 'Total Amenities', value: totalAmenities, icon: CheckCircle },
            { label: 'Rooms with Amenities', value: uniqueRooms, icon: MapPin, accent: goldDeep },
        ...
        <div style={{
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input
              type="text"
              placeholder="Search by ID, room, amenity name, or description…"
              value={searchTerm}
        ...
        {loading && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '70px 0' }}>
            <div style={{
              ...
              borderRadius: '50%', animation: 'ra-spin 0.8s linear infinite',
            }} />
        ...
        {!loading && filteredRecords.length > 0 && (
          <div className="ra-table-wrap" style={{
            background: '#FFFFFF', border: `1px solid ${line}`, borderRadius: 18,
            overflow: 'hidden', boxShadow: shadowCard,
          }}>
            <div style={{ overflowX: 'auto' }}>
              <table className="ra-table" style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1100 }}>
                <thead>
                  <tr style={{ background: ivory, borderBottom: `1px solid ${line}` }}>
                    {['ID', 'Room ID', 'Room', 'Amenity Name', 'Description', 'Created By', ''].map((h, i) => (
                      <th key={i} style={{
                ...
                <tbody>
                  {filteredRecords.map((a, idx) => (
                    ...
                    >
                      ...
                      <td data-label="" className="ra-empty-label" style={{ padding: '14px 18px' }}>
                        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                          {permissions.update_room_amenity && (
                            <button
                              onClick={() => openEdit(a)}
                              title="Update"
                              style={{
                          ...
                          {permissions.delete_room_amenity && (
                            <button
            ...
            {/* Pagination */}
            {totalPages > 1 && (
              <div className="ra-pagination" style={{
        ...
        {!loading && filteredRecords.length === 0 && (
          <div style={{
            ...
            {permissions.create_room_amenity && !searchTerm && (
              <button
                onClick={openCreate}
                style={{
      ...
      {modalOpen && (
        <Modal
          ...
          subtitle={editingAmenity ? `Editing amenity for ${editingAmenity.room_name_en || editingAmenity.room?.name_en}` : 'Add a new amenity to a room'}
          onClose={closeModal}
          wide
        >
...
export default RoomAmenitiesCom;