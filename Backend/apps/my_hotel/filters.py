...
class RoomFilter(filters.FilterSet):
    ...
    class Meta:
        model = Room
        fields = ['hall', 'occupied', 'created_at']
...
class BookingFilter(filters.FilterSet):
    ...
    class Meta:
        model  = Booking
        fields = ['status', 'time_slot', 'hall', 'customer', 'date']
...
class RoomBookingFilter(filters.FilterSet):
    ...
    def filter_search(self, queryset, name, value):
        return queryset.filter(
            ...
            Q(event_type_en__icontains=value)    |
            Q(event_type_ar__icontains=value)    |
            Q(room__name_en__icontains=value)    |
            Q(customer__name_en__icontains=value)
        )
...
class PaymentFilter(filters.FilterSet):
    search = filters.CharFilter(method='filter_search')
    booking = filters.NumberFilter(field_name='booking_id')
    room_booking = filters.NumberFilter(field_name='room_booking_id')
    payment_method = filters.CharFilter(field_name='payment_method', lookup_expr='iexact')
    date_to = filters.DateFilter(field_name='payment_date', lookup_expr='lte')
    date_from = filters.DateFilter(field_name='payment_date', lookup_expr='gte')
    ...
    class Meta:
        model = Payment
        fields = ['booking', 'room_booking', 'payment_method', 'payment_date']

    def filter_search(self, queryset, name, value):
        return queryset.filter(
            Q(booking__booking_code__icontains=value) |
            Q(room_booking__booking_code__icontains=value) |
            Q(notes__icontains=value)
        )
...
class RoomPricingFilter(filters.FilterSet):
    ...
    class Meta:
        model = RoomPricing
        fields = ['room', 'time_slot', 'valid_from']
...
class BookingServiceFilter(filters.FilterSet):
    ...
    class Meta:
        model = BookingService
        fields = ['booking', 'room_booking', 'created_at']
...
# ROOM AMENITY
# ─────────────────────────────────────────────────────────────────────
class RoomAmenityFilter(filters.FilterSet):
    search = filters.CharFilter(method='filter_search')
    room = filters.NumberFilter(field_name='room_id')
    date_to = filters.DateFilter(field_name='created_at', lookup_expr='lte')
    date_from = filters.DateFilter(field_name='created_at', lookup_expr='gte')
