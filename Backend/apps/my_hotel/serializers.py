...
from .models import (
    ...
    Room, RoomBooking, RoomPricing, RoomAmenity,
)
...
class RoomSerializer(RoomOccupancyMixin, serializers.ModelSerializer):
    occupied = serializers.SerializerMethodField()
    upcoming_occupied_dates = serializers.SerializerMethodField()
...
class BookingSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        booking = super().create(validated_data)
        # occupied / occupied_dates are computed live on Hall (see is_occupied_today,
        # get_upcoming_bookings) — nothing to write here. Just keep the denormalized count fresh.
        booking.hall.recalculate_booking_count()
        return booking
...
class RoomBookingSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        room_booking = super().create(validated_data)
        room_booking.room.recalculate_booking_count()
        return room_booking

    def update(self, instance, validated_data):
        old_room = instance.room
        room_booking = super().update(instance, validated_data)

        if old_room != room_booking.room:
            old_room.recalculate_booking_count()
...
class RoomBookingListingSerializer(serializers.ModelSerializer):
...
class RoomRevenueReportRowSerializer(serializers.Serializer):
    booking_code = serializers.CharField()
    room_name_en = serializers.CharField()
    customer_name = serializers.CharField()
    date = serializers.DateField()
    status = serializers.CharField()
    total = serializers.DecimalField(max_digits=12, decimal_places=2)
...
class PaymentSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        room_booking = attrs.get('room_booking') or (self.instance.room_booking if self.instance else None)

        if bool(booking) == bool(room_booking):
            raise serializers.ValidationError('Payment must be linked to exactly one of booking or room_booking')
        return attrs

    def to_representation(self, instance):
        data = super().to_representation(instance)
        data['booking_code'] = instance.booking.booking_code if instance.booking else (
            instance.room_booking.booking_code if instance.room_booking else None
        )
        data['payment_method_display'] = instance.get_payment_method_display()
        data['created_by_name'] = instance.created_by.get_full_name() if instance.created_by else None
        data['updated_by_name'] = instance.updated_by.get_full_name() if instance.updated_by else None
...
class PaymentListingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Payment
        fields = ['id', 'booking', 'room_booking', 'booking_code', 'amount', 'payment_method', 'payment_date', 'notes', 'created_at']

    def get_booking_code(self, instance):
        if instance.booking:
            return instance.booking.booking_code
        if instance.room_booking:
            return instance.room_booking.booking_code
        return None
...
class RoomPricingSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if room and time_slot and valid_from:
            clashing = RoomPricing.objects.filter(
                room=room, time_slot=time_slot, valid_from=valid_from, deleted=False
            )
            if self.instance:
                clashing = clashing.exclude(id=self.instance.id)
            if clashing.exists():
                raise serializers.ValidationError(
                    'Pricing for this room, time slot, and valid_from date already exists'
                )
        return attrs
...
class RoomPricingListingSerializer(serializers.ModelSerializer):
    room_name_en = serializers.CharField(source='room.name_en', read_only=True)

    class Meta:
        model = RoomPricing
        fields = ['id', 'room', 'room_name_en', 'time_slot', 'base_price', 'valid_from', 'valid_until', 'created_at']
...
class BookingServiceSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if bool(booking) == bool(room_booking):
            raise serializers.ValidationError('BookingService must be linked to exactly one of booking or room_booking')
        return attrs
...
class BookingServiceListingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = BookingService
        fields = ['id', 'booking', 'room_booking', 'booking_code', 'service_name_en', 'service_name_ar', 'cost', 'notes', 'created_at']
...
class RoomAmenityListingSerializer(serializers.ModelSerializer):
    room_name_en = serializers.CharField(source='room.name_en', read_only=True)

    class Meta:
        model = RoomAmenity
        fields = ['id', 'room', 'room_name_en', 'name_en', 'name_ar', 'description_en', 'description_ar', 'created_at']